...
// Alloc physical page for virtual address
int
vma_alloc(uint64 va)
{
  ...
  void* pa = kalloc();
  if(pa == 0) 
    panic("vma_alloc: kalloc");
  memset(pa, 0, PGSIZE);

  // read file from disk
  begin_op();
  ilock(v->f->ip);
  readi(v->f->ip, 0, (uint64)pa, v->offset + PGROUNDDOWN(va - v->start), PGSIZE);
  iunlock(v->f->ip);
  end_op();

  // map virtual address to physical address
  if(mappages(p->pagetable, va, PGSIZE, (uint64)pa, PTE_U | PTE_W | PTE_R) < 0) 
    panic("vma_alloc: mappages");
    ...
    for (a = va;a < va + nbytes;a += PGSIZE) {
        ...
        if (*pte & PTE_V) {
            ...
            if ((*pte & PTE_D) && (v->flags & MAP_SHARED)) {        // 将修改写回磁盘
                begin_op();
                ilock(v->f->ip);
                uint64 aoff = a - v->start;                       // 相对于vma的偏移量
                if (aoff < 0)
                    writei(v->f->ip, 0, pa + (-aoff), v->offset, PGSIZE + aoff);        // 第一页是不满PGSIZE的一个页
                else if (aoff + PGSIZE > v->length)
                    writei(v->f->ip, 0, pa, v->offset + aoff, v->length - aoff);            // 最后一页是不满PGSIZE的一个页
                else
